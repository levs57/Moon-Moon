  ...
  for NovaAugmentedCircuit<G, SC>
{
  fn synthesize<CS: ConstraintSystem<<G as Group>::Base>>(
    ...
    let run = if !u.W_exposed.is_empty() {
      let mut runs = vec![];
      for (w_exposed_i, run_i) in u.W_exposed.iter().zip(u.run.iter()) {
        let mut ro_run = G::ROCircuit::new(self.ro_consts.clone(), 4); // new: it needs to absorb w0 and u.X2. Currently suboptimal, because I'm decomposing u.X2 into bits again
