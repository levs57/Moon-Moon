  ...
  SpartanSNARK<G, EE, C>
{
  ...
  pub fn prove(pk: &SpartanProverKey<G, EE>, sc: C, z_i: &[G::Scalar]) -> Result<Self, NovaError> {
    ...
    let (u_relaxed, w_relaxed) = (
      RelaxedR1CSInstance::from_r1cs_instance_unchecked(&u.comm_W, &u.comm_W_exposed, &u.run, &u.X),
  ...
  pub fn verify(&self, vk: &SpartanVerifierKey<G, EE>, io: &[G::Scalar]) -> Result<(), NovaError> {
    ...
    let u_relaxed =
      RelaxedR1CSInstance::from_r1cs_instance_unchecked(&self.comm_W, &vec![], &vec![], io);
